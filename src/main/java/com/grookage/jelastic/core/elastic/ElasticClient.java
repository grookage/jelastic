...
import com.grookage.jelastic.core.utils.ElasticClientUtils;
...
public class ElasticClient {
    ...
    private void initializeClient() throws CertificateException, KeyStoreException, IOException, NoSuchAlgorithmException, KeyManagementException {
        ...
        if(null != jElasticConfiguration.getAuthConfiguration()){
            final var sslContext = ElasticClientUtils.getSslContext(jElasticConfiguration);
            final var credentialsProvider = ElasticClientUtils.getAuthCredentials(jElasticConfiguration);
        ...
        final var settings = ElasticClientUtils.getSettings(jElasticConfiguration);
        if(null != settings){
